...
public:
    long long totalCost(vector<int>& costs, int k, int candidates) {
        long long res = 0;
        int left = candidates, right = max((int)costs.size() - candidates, candidates) - 1;
        priority_queue<int, vector<int>, greater<int>> pqLeft(costs.begin(), costs.begin() + candidates);
        priority_queue<int, vector<int>, greater<int>> pqRight(costs.begin() + right + 1, costs.end());
        pqLeft.push(1e5 + 10);
        pqRight.push(1e5 + 10);
        while (k--) {
